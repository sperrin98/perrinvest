...
const SilverReturns = () => {
    ...
    // Fetch data from the Flask backend for Silver returns
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_API_URL}/returns/2`);  // Fetching silver data
                if (!response.ok) {
    ...
    return (
        <div className="returns-container">
            <h1 className="returns-header">Annual Silver Price Returns</h1>
