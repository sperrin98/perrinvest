...
function MeanSeasonality() {
  ...
  const statCards = useMemo(() => {
    ...
    const bestMonth = rows.reduce((best, row) => {
      if (!best) return row;
      return row.geometric_mean_monthly_return_pct > best.geometric_mean_monthly_return_pct
        ? row
        : best;
    }, null);
    ...
    const worstMonth = rows.reduce((worst, row) => {
      if (!worst) return row;
      return row.geometric_mean_monthly_return_pct < worst.geometric_mean_monthly_return_pct
        ? row
        : worst;
    }, null);
    ...
    <div className="msx-page">
      <div className="msx-container">
        <aside className="msx-sidebar">
          ...
          {loadingSecurities ? <div className="msx-hint">Loading commodities…</div> : null}
          ...
          <button className="msx-load-btn" onClick={fetchMeanSeasonality} type="button">
            Load Chart
          </button>
        </aside>
          ...
          {statCards && (
            <div className="msx-stats-row">
              <div className="msx-stat-card">
                ...
                <span className="msx-stat-value">{statCards.bestMonth?.month_name || "-"}</span>
                <span className="msx-stat-sub">
                  {Number.isFinite(statCards.bestMonth?.geometric_mean_monthly_return_pct)
                    ? `${statCards.bestMonth.geometric_mean_monthly_return_pct.toFixed(2)}%`
                    : "-"}
                </span>
              </div>
              ...
              <div className="msx-stat-card">
                ...
                <span className="msx-stat-value">{statCards.worstMonth?.month_name || "-"}</span>
                <span className="msx-stat-sub">
                  {Number.isFinite(statCards.worstMonth?.geometric_mean_monthly_return_pct)
                    ? `${statCards.worstMonth.geometric_mean_monthly_return_pct.toFixed(2)}%`
                    : "-"}
                </span>
              </div>

              <div className="msx-stat-card">
                <span className="msx-stat-label">Average Hit Rate</span>
                <span className="msx-stat-value">
                  {Number.isFinite(statCards.avgHitRate)
                    ? `${statCards.avgHitRate.toFixed(1)}%`
                    : "-"}
                </span>
                <span className="msx-stat-sub">Across all months</span>
              </div>
            </div>
          )}
